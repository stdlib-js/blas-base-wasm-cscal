...
tape( 'a module instance has an `ndarray` method which scales elements from `cx` by `ca`', function test( t ) {
	var actualX;
	var viewX;
	var cap;
	var cxe;
	...
	mod.ndarray( 3, cap, cxp, 1, 0 );

	actualX = new Complex64Array( 3 );
	mod.read( cxp, actualX );
	viewX = new Float32Array( actualX.buffer );
	isApprox( t, viewX, cxe, 0 );

	t.end();
});

tape( 'a module instance has an `ndarray` method which supports a `cx` stride', function test( t ) {
	var actualX;
	var viewX;
	var xbuf;
	var cap;
	...
	mod.read( cxp, actualX );
	viewX = new Float32Array( actualX.buffer );
	isApprox( t, viewX, cxe, 1 );

	t.end();
});

tape( 'a module instance has an `ndarray` method which supports a negative `cx` stride', function test( t ) {
	var actualX;
	var viewX;
	var xbuf;
	var cap;
...
tape( 'a module instance has an `ndarray` method which supports a `cx` offset', function test( t ) {
	var actualX;
	var viewX;
	var xbuf;
	var cap;
...
tape( 'if provided an `N` parameter less than or equal to `0`, a module instance has an `ndarray` method which leaves the input array unchanged', function test( t ) {
	...
	out = mod.ndarray( 0, cap, cxp, 1, 0 );
	t.strictEqual( out, cxp, 'returns expected value' );

	out = mod.ndarray( -1, cap, cxp, 1, 0 );
	t.strictEqual( out, cxp, 'returns expected value' );
